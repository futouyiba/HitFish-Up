...
> **2026-09-17 追加**：上面的「建议范围：只做行带这一层」已被 Design Owner 的决定取代 ——
> 改为**行 + 行内部一起做**，且**七行全用动态让位**（不保留方案一）。下面是为此做的前置探针。

---

# 落地前置探针（P1–P7）

`probe-nest.drawio` + `probe_nest_build.py`（新的容器形态）。同一套驱动：pinned viewer +
合成鼠标事件 + 回读 `getState` 几何。参数：`stackBorder=10`、`stackSpacing=10`。

| # | 问题 | 结论 |
|---|---|---|
| **P1** | `movable=0` 的子块是否被排除出栈 | **是**。见下 |
| **P2** | 负的父相对坐标能否渲染 | **能** |
| **P3** | 水平容器的高度是否永不重算 | **是（高度汇）** |
| **P4** | 水平嵌在垂直里是否稳定 | **稳定，40 轮零漂移** |
| **P5** | 垂直容器上的 `marginLeft` | **按预期缩进** |
| **P6** | `hide` 一个容器 id 是否级联隐藏子孙 | **级联** |
| **P7** | 一次点击是否在同一事务里跑完布局 | **是** |

### P1 / P2 — `movable=0` 侧钉（实测）

`P1BOX`（垂直容器）里放 `P1A`(h40)、`P1B`(h40) 和一个 `movable=0` 的 `P1PIN`（局部 x = **−230**）。
跑一次布局后：

| | 局部几何 | 绝对几何 |
|---|---|---|
| P1A | `10,10,280,40` | `310,70,…` |
| P1B | `10,60,280,40` | `310,120,…` |
| **P1PIN** | **`-230,10,200,40`（没被动过）** | **`70,70,…`** |
| P1BOX | `300,60,300,**110**` | 同 |

`110 = 2×10 + (40+10+40) + 10` → **钉子没有计入父容器高度**；两个正常子块按无钉子那样叠放；
钉子的绝对位置 = 父容器 + 局部偏移 → **随父容器一起移动**。负的局部坐标照常渲染。

### P3 / P4 — 水平容器是「高度汇」（这条改掉了原设计）

`P3H` 是 `horizontalStack=1` 的容器，里面 `P3L` / `P3R` 两个垂直子容器；`P3TAIL` 排在 `P3H` **之后**。
往 `P3R` 里放两个隐藏子块，然后**点一次**显示它们：

| | 布局后 | 显示 P3R2+P3R3 后 | 再隐藏 |
|---|---|---|---|
| P3R 高 | 50 | **130**（它自己长大了） | 50 |
| **P3H 高** | 130 | **130（完全没变）** | 130 |
| **P3TAIL 的 y** | 210 | **210（完全没动）** | 210 |

**结论：水平容器的高度引擎永不重算，所以它下面的东西永远不会被推动。**
（`updateParentGeometry` 对水平栈只写宽度、对垂直栈只写高度。）

由此得到整个设计的硬规则：

> **重排只能沿"垂直容器的连续链"向上传播。链上出现一个水平容器，重排就断在那里。**
> 所以**凡是"展开时必须推动下方"的东西，都必须是垂直栈里的兄弟**；水平容器只能装
> **叶子 / 不可再展开的内容**，其高度在发射时钉死。

P4 补充：虽然它阻断重排，但它本身**是稳定确定的** —— 40 轮 show/hide 零漂移、精确回到发射值。

### P5 — `marginLeft`（实测）

`P5BOX` 加 `marginLeft=80`：首个子块 x = `90 = border(10) + 80`；子块宽 `300 = 400 − 2×10 − 80`；
容器高 `130 = 2×10 + (30+10+30+10+30) + 10` —— 缩进不影响那条高度公式。

### P6 — 隐藏会级联（实测）

只对容器 id 发 `hide {"cells":["P6BOX"]}`（**不枚举子孙**）：`P6BOX`、孙容器 `P6GRP`、
曾孙 `P6X` **全部**变成不渲染；再 `show P6BOX` 三个都精确复原。
→ toggle payload 不需要枚举子孙 cell；但**边不是容器的子孙**，仍要显式列。

### P7 — 一次点击就够

上面 P3 的"显示两个子块"只调了一次 `handleCustomLink`，`P3R` 就长大了 —— 布局在同一个编辑事务里跑完。

### 高度公式复核（与实测对齐）

- 垂直容器：`H = 2·border + Σh + spacing·(n−1)`；第 i 个子块 `y = border + Σ_{j<i} h_j + spacing·i`；
  子块 `x = border + marginLeft`，宽被撑成 `W − 2·border − marginLeft`。
- 水平容器：`W = 2·border + marginLeft + Σw + spacing·(n−1)`；第 i 个子块 `x = border + marginLeft + Σ_{j<i} w_j + spacing·i`；
  **`H` 必须发射时钉死**。
- 补充实测：水平容器里的子块**若本身也是容器**，其高度来自**它自己的内容**，不会被父容器的 `fill` 拉平
  （`P3L` 发射 110，布局后变 90 = 它自己的内容高）。所以水平带的高度要按"最高的那个子块"来钉。
