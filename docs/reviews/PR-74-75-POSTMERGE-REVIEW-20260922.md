...
**补记(2026-09-22,本记录落笔之后)**:主代理就此类路径裁了一条**短路径 carve-out** —— 逐字「纯删除类(死文件＋全树引用 0＋前提独立核过)可只走『独立前提核 ＋ 合并后 intg 留痕』,内容类 PR 仍走四段」。⇒ `#74`／`#75` 属**命中短路径**,而非漏审。该裁定是**主代理层级、非 Owner 裁决**;上文「是否受理该路径归 Owner」的挂起项**不因它消解**。本段为后置补记,不改上文在 `cbb7d019` 上的读数。
