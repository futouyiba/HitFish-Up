...
- Export time：**按组两区间** —— **9 帧**：`未落/待核；仅有定位盘存上界 [2026-09-22T11:46:50Z, 2026-09-23T02:59:04Z]`（无像素校验窗口，故9帧不标consistent）；**7 帧（重导）**：`2026-09-23T05:12:55Z–05:13:10Z`（逐帧服务端导出/核验时点；本地无更精确日志，不猜测）。
- Proof boundary：the PNG proves only static pixels at export time; it does not prove runtime behavior or persistence.
