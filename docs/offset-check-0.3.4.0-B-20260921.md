...
**本轮我自己的三处锚偏了，由主代理复核时抓出**：

| 我写的（**原写法**，此处留痕） | 实际 |
| --- | --- |
| `app.ts:858–885` | **`src/ui/app.ts:870–891`**（`src/ui/app.ts:858` 那行是 `onRoleEdit`，**不是本函数**；注释块 864–869、函数体 870–891） |
| `persistence.ts:1117–1119` | **`src/data/persistence.ts:1117–1120`**（`setPatchFor`） |
| `persistence.ts:1396` | **`src/data/persistence.ts:1396`**（`httpFileStore`） |

⇒ **三处已改正，并给全表补了内容锚与相对全路径**（本节即那次自查的记录）。
