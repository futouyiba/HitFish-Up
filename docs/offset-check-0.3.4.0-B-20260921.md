...
> **怎么得的**：脚本 `/tmp/anchor-check.mjs` 对本文件实跑（**自带四条非空转自检**：行号越界必红／已知正确必绿／**本报告那处真缺陷必红**／路径不存在必红；**不过即退 2、什么都不做**），逐锚在 `bae6e6f`→`3cb3074`→`ecbd1df`→`b003dfe` 四个 base 上解析「这一行属于哪个声明」；**区间锚与两处「注释声称」由人工逐字复核**（原因见下）。
> ⚠️ **脚本的已知局限**：它取的是「**最近的声明**」（从该行**向上**找）。⇒ **函数体起始行在锚点之下时，它会把该行归给上面那个函数**。本表里 `app.ts:858–885` 与 `app.ts:1240–*` 就是这种，**已人工改正**。

| 锚 | 解析于 | 依据／在别处是什么 |
| --- | --- | --- |
| `src/data/persistence.ts:872` | **`3cb3074`** | 「时段形状不同」的写入分支；`bae6e6f` 上那一行是 `if` |
| `src/data/persistence.ts:1088` | **`3cb3074`** | 「无档案被读成全 0」的绕行；`bae6e6f` 上是 `if` |
| `src/data/persistence.ts:1278–1287` | **`3cb3074`** | 断链检查；`bae6e6f` 上是 `TIME_PERIOD` |
| `src/data/persistence.ts:1396` ／ `:1396–1410` | **`3cb3074`** | `httpFileStore`；★ **`bae6e6f` 上该文件没那么长 ⇒ 越界取不到** |
| `src/core/contract.ts:111–113` | **`3cb3074`** | 「**四个槽位一律可缺席**」那段注释（`#27`）；★ **`bae6e6f` 上这三行只有 `timePeriod: TimePeriodProfile \| null`** —— 即**只有时段可空** |
| `src/ui/app.ts:870–891` | **`ecbd1df`** | `onApplyPeriodPreset` 的函数体；`bae6e6f`/`3cb3074` 上该区间还没长到那里 |
| `src/editor/policyTemplate.ts:108–110` | **`ecbd1df`** | `rawRoleOf` —— 本报告**时点锚行**里的新锚 |
| `src/editor/refConsumers.ts:53` | **`bae6e6f`**（**基线系**） | 报告声称的是 `OwnerGranularity`，**它只在 `bae6e6f`/`3cb3074` 上成立**；`ecbd1df` 起该行是 `consumerKeyOf` |
| `src/editor/session.ts:123–128` | **`bae6e6f`**（**基线系**） | 报告声称的是**一段注释**；**逐字核过：那六行注释在 `bae6e6f` 上确实在**（`3cb3074` 起该区间是别的代码） |
| `src/ui/app.ts:858–885` | **`bae6e6f`**（**基线系**） | ★ 人工改正：`bae6e6f` 上 = **注释块 858–863 ＋ 函数体 864–885**（正是 `onApplyPeriodPreset`）。脚本把它归给 `onToggleComponent` 是**向上找**的假象 |
| `src/ui/app.ts:1240–1265` ／ `:1240–1275` | **`bae6e6f`**（**基线系**） | ★ 人工改正：行区间在四个 base 上都落在同一函数体，但**函数名在 `ecbd1df` 起由 `runWriteBack` 改为 `onRunWriteBack`** ⇒ 锚对、**符号名已变** |
