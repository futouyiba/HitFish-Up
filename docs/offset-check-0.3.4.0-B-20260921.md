...
> **怎么得的**：脚本 `/tmp/anchor-check.mjs`（**scratch，未入库，也不打算入库**）对本文件实跑，逐锚在 `bae6e6f`→`3cb3074`→`ecbd1df`→`b003dfe` 四个 base 上**同时打出「那一行原文」与「它最近的声明」**；两处「注释声称」由**人工逐字复核**（见 C）。
>
> ⚠️ **脚本的三条已知局限（都已在 v3 修掉，此处留痕）**：
> ① 它取的是「**最近的声明**」（从该行**向上**找）⇒ **函数体起始行在锚点之下时，会归给上面那个函数**（`src/ui/app.ts:858–885`、`src/ui/app.ts:1240–1275` 就是这种，**已人工改正**）；
> ② ★ **它会把控制关键字当成声明捕获** ⇒ `if (f === 'TIME_PERIOD' && …)` 被报成 owner ＝ 裸 `if`。**这不是「该行不属于声称的符号」，是这个 token 本身是假值** —— 本表初稿据此把 `872`／`1088` 的归属**判反**（见 B）。v3 加了一张「不算声明」的关键字表，并**把「那一行原文」一起打出来**：报「那一行实际是什么」比报「最近的声明是谁」更难被误读；
> ③ ★ **「是不是跨修订」的判据必须打在字节上，不能打在同一层的代理量上**：v1 拿 `owner` 当判据 ⇒ **漏报**了 `src/data/persistence.ts:484–489`（四个 base 的 owner 全是 `at`，而那一行是 `}` vs `typeof rec.refs…`）。改用字节后，跨修订数 **14 → 16**。
>
> v3 的 **8 条自检**是本表可信度的全部依据 ⇒ **逐条写在下面**；判据本身可复现，**不依赖那个 scratch 文件**。

**★ 8 条自检 —— 逐条给判据与方向**（「必须红」＝不满足就该拒；「必须绿」＝满足就该过）。**8 条不过即退 2、什么都不做** —— 宁可拒，不出读数。

| # | 自检判据 | 具体输入（`bae6e6f` ＝ 本报告声明的基线） | 方向 |
| --- | --- | --- | --- |
| ① | 行号越界 | 锚 `src/data/persistence.ts:999999` | **必须红** —— 须报「越界」 |
| ② | 已知正确 | 锚 `src/data/persistence.ts:1061` ＋ 声称 `setPatchFor` | **必须绿** —— 须命中 |
| ③ | **本报告那处真缺陷** | 锚 `src/data/persistence.ts:1117` ＋ 声称 `setPatchFor` | **必须红** —— `bae6e6f` 上该行是 `...state,`，**不许命中** |
| ④ | 路径不存在 | 锚 `src/data/nope.ts:1`（**该路径故意不存在**，是探针） | **必须红** —— 须报「路径不存在」 |
| ⑤ | 控制关键字不得当声明 | 锚 `src/data/persistence.ts:872`（该行本身是 `if (f === 'TIME_PERIOD' && …)`） | **必须绿** —— 它必须走到**函数名**，`owner` **不得**是 `if` |
| ⑥ | 语句行须被标出 | 同上锚 | **必须绿** —— 该行须被标成「**不是**声明」 |
| ⑦ | **字节变必报跨修订** | 锚 `src/data/persistence.ts:484–489` | **必须绿** —— 四个 base 的 `owner` 相同（都是 `at`）而**字节不同**（`}` vs `typeof rec.refs…`）⇒ 必须判「跨修订」 |
| ⑧ | 字节不变不得报跨修订 | 锚 `src/data/writeBackClient.ts:50` | **必须绿** —— 四个 base 字节全同 ⇒ 必须判「**不**跨修订」 |

★ **⑦⑧ 是一对反向判据**：只测「该报的报了」**不够** —— **一律报也是永远绿**。⑧ 的存在使「无脑全报跨修订」这种退化实现**当场变红**。
★ **每条的「能红」是实测过的，不是声称的** —— 用**定向突变**逐条证伪，且**每个突变只让它该红的那条红**：
· 清空「不算声明」的关键字表 ⇒ **⑤ 红**（其余不动）；
· 跨修订判据退回「最近的声明」⇒ **⑦ 红**；
· 跨修订判据改成「按次序恒不同」⇒ **⑧ 红**；
· 「是不是声明」的判定改成恒真 ⇒ **⑥ 红**。
