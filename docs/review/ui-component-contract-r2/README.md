...
| `img/` | **随包图像 5 张**（整帧、不含 annotation；**4 张为 `scale=1`，`INHERIT` 那张为 `scale=2.5`**；型号与指纹见 `figma-current.md` §六）。★ **引用 Policy 现状请用 `108:364` 那两张**（`SET` 态与 **`INHERIT` 态**，**同轮导出**）—— `176:283` 是**母件**、不在作者面上（§六 有补拍说明）。 |
