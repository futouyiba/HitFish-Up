...
- Rail 的展开 / 折叠只是 session UI state，不产生 durable authoring entity。作者可以主动折叠当前 Fish；这不会改变当前 Subject，中栏标题仍持续显示当前 Fish + Subject。Fish 行的 disclosure arrow 只负责展开 / 折叠，不触发 Subject mutation；点击 Fish label 则展开该 Fish 并明确进入其「基础习性」。不维护 `lastSubjectByFish` 之类隐藏恢复状态。
