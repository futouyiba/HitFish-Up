...
- Rail 的展开 / 折叠只是 session UI state，不产生 durable authoring entity。作者可以主动折叠当前 Fish；这不会改变当前 Subject，中栏标题仍持续显示当前 Fish + Subject。再次点击该 Fish label 时恢复其 session 内最近选择的 child Subject；没有记录则进入「基础习性」。Fish 行的 disclosure arrow 只负责展开 / 折叠，不触发 Subject mutation。
