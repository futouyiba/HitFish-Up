...
- 不得把来源、物种层、本层画成连续数学流水线（整层替换、不是叠加）。（《编辑器持久层契约》§3.3；《编辑器心智模型与 IA》§1）
- 已覆盖但数值与来源相同，仍读作已覆盖（界面读记录，不按值差）。（《编辑器持久层契约》§3.5）

**durable 语义**：
- `CLEAR` 尽管 Effective Value 可能等于来源原值，仍是明确的 durable 本层操作，计入本层操作数。（《编辑器持久层契约》§3.3 `op` 行；冻结卡 `A①-卡2`）
